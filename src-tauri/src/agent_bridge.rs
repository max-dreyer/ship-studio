...
const TOOLS: &[ToolDef] = &[
    ...
    ToolDef {
        ...
        description: "List the user's other Ship Studio projects on this machine: name, absolute path, whether each is open in Ship Studio, and its reserved local preview port when open. Use before studio_ask to find the right project, or to discover a sibling app's local URL instead of asking the user for it.",
        timeout_secs: DEFAULT_TOOL_TIMEOUT_SECS,
        input_schema: || json!({ "type": "object", "properties": {} }),
    },
    ToolDef {
        name: "studio_ask",
        description: "Ask a question to another local Ship Studio project. A read-only agent session runs inside that project and answers from its real code, config, and conventions — use this instead of guessing about a sibling project or asking the user to copy-paste context between projects. It takes a while (often 30s-2min) and uses the user's normal agent plan, so ask substantial questions (API shapes, behavior, architecture), not things a file read would answer. The user watches the exchange live in Ship Studio.",
